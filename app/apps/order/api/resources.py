...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def get_count_items_active_order(self, request):
        active_order = self.request.user.order_set.filter(status='new').first()
    ...
    def get_active_order(self, request):
        ...
        if active_order is None:
            active_order = Order()
            active_order.creation_date = timezone.now()
            active_order.created_by = self.request.user
            active_order.save()
    ...
    @action(methods=['get'], detail=False, name="get_confirmed_orders")
    def get_confirmed_orders(self, request):

        if self.request.user.order_set.filter(status='confirmed', total__gt=0).exists():
            confirmed_orders = self.request.user.order_set.filter(status='confirmed')
            confirmed_orders_list = []
            for confirmed_order in confirmed_orders:
                serializer = OrderSerializer(confirmed_order)

                confirmed_order_dict = {
                    'order': serializer.data,
                    'items': confirmed_order.items(request),
                }

                confirmed_orders_list.append(confirmed_order_dict)

            return Response(confirmed_orders_list)
        else:
            return Response({'status': 'Orders not found'})

    @action(methods=['get'], detail=False, name="confirmed_order")
    def confirmed_order(self, request):
        order = self.request.user.order_set.filter(status='new').first()
